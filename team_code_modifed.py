...
def training_code_efficientNet(data_directory, model_directory):
    ...
    classes1, leads, classifier = our_clsf_model(filename,data_directory,train_data_chl12,leads)
    
    feature_indices = [twelve_leads.index(lead) for lead in leads]
    features = data[:, feature_indices]
    imputer=SimpleImputer().fit(features)
    save_model(filename, classes1, leads, imputer, classifier)
    ...
    classes1, leads, classifier = our_clsf_model(filename,data_directory,train_data_chl6,leads)
    ...
    classes1, leads, classifier = our_clsf_model(filename,data_directory,train_data_chl3,leads)
    ...
    classes1, leads, classifier = our_clsf_model(filename,data_directory,train_data_chl2,leads)
...
def get_signal_spectrum(header, recording, leads,data_directory, label, k):
    ...
    mapped_scored_labels = np.array(list(csv.reader(open('/content/drive/MyDrive/evaluation-2021/dx_mapping_scored.csv'))))
    data_directory_image = data_directory
    b = "Database"
    data_directory_image = data_directory_image.replace(b,"Database_Image")
    # Reorder/reselect leads in recordings.
    available_leads = get_leads(header)
    indices = list()
    for lead in leads:
...
def build_model(num_classes,IMG_SIZE):
    ...
    #optimizer = tf.keras.optimizers.SGD(learning_rate=0.001,momentum=0.3)
...
def our_clsf_model(filename,data_directory,train_data_sp,leads):
    ...
    for train_index, val_index in skf.split(np.zeros(len(Y)),Y):
        ...
        #imputer=SimpleImputer().fit(train_data_generator)
            ...
            # CREATE CALLBACKS
    		checkpoint = tf.keras.callbacks.ModelCheckpoint(filename, monitor='val_accuracy', verbose=1,save_best_only=True, mode='max')
